...
int numCommonLinks(const unordered_set<string>& curr_set,
  ...
  return std::count_if(curr_set.begin(), curr_set.end(), [&](string ele) {
    return target_set.find(ele) != target_set.end();
  });
}
...
vector<string> findWikiLadder(const string& start_page,
  ...
  // compile.
  std::priority_queue<vector<string>, vector<vector<string>>, decltype(cmp_fn)>
      queue(cmp_fn);
