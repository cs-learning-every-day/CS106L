...
template <typename K, typename M, typename H>
typename HashMap<K, M, H>::const_iterator HashMap<K, M, H>::find(
    const K& key) const {
...
template <typename K, typename M, typename H>
std::pair<typename HashMap<K, M, H>::const_iterator, bool>
HashMap<K, M, H>::insert(const value_type& value) const {
  return static_cast<std::pair<const_iterator, bool>>(const_cast<HashMap<K, M, H>*>(this)->insert(value));
}

template <typename K, typename M, typename H>
typename HashMap<K, M, H>::node_pair HashMap<K, M, H>::find_node(
    const K& key) const {
  size_t index = _hash_function(key) % bucket_count();
...
size_t HashMap<K, M, H>::first_not_empty_bucket() const {
  ...
  auto found =
      std::find_if(_buckets_array.begin(), _buckets_array.end(), isNotNullptr);
  return found - _buckets_array.begin();
}

template <typename K, typename M, typename H>
typename HashMap<K, M, H>::iterator HashMap<K, M, H>::make_iterator(
    node* curr) {
  if (curr == nullptr) {
...
typename HashMap<K, M, H>::iterator HashMap<K, M, H>::erase(
  ...
  erase(pos++->first);
  return make_iterator(pos._node);  // unfortunately we need a regular iterator,
...
template <typename K, typename M, typename H>
void HashMap<K, M, H>::rehash(size_t new_buckets) const {
  const_cast<HashMap<K, M, H>*>(this)->rehash(new_buckets);
}
...
HashMap<K, M, H>::HashMap(InputIt first, InputIt last, size_t bucket_count,
                          const H& hash)
...
HashMap<K, M, H>::HashMap(std::initializer_list<value_type> init,
                          size_t bucket_count, const H& hash)
...
HashMap<K, M, H>::HashMap(const HashMap& other)
  ...
  for (size_t i = 0; i < other.bucket_count(); ++i) {
    ...
    node* prev = nullptr;
    while (curr != nullptr) {
      auto temp = new node(curr->value);
      if (prev == nullptr) {
        _buckets_array[i] = temp;
      } else {
        prev->next = temp;
      }
      prev = temp;
...
HashMap<K, M, H>& HashMap<K, M, H>::operator=(const HashMap& other) {
  if (this != &other) {
    ...
    std::swap(_hash_function, temp._hash_function);
    std::swap(_buckets_array, temp._buckets_array);
  }
  return *this;
}

template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(HashMap&& other) noexcept
    : _size{other._size},
